# Deploying MachinistPro to Vercel (step by step)
...
| `SUPABASE_SERVICE_ROLE_KEY` | the service-role key from Lovable Cloud |
...
Generate `APP_PEPPER` once and never change it:
...
- `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` — the server reads/writes users,
