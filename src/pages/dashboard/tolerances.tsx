...
import {
  calcFit, COMMON_FITS, SHAFT_LETTERS, AVAILABLE_GRADES,
  GDT_SYMBOLS, GDT_CATEGORIES,
  SURFACE_FINISHES, PREFERRED_NUMBERS,
  type FitResult,
} from "@/lib/tolerances";
...
function fmt(n: number, d = 3) { return n.toFixed(d).replace(/\.?0+$/, ""); }
...
function FitsTab() {
  ...
  const fitColor = result?.fitType === "clearance" ? "green" : result?.fitType === "interference" ? "red" : "amber";
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card variant="solid" padding="md" className="border-dark-600 space-y-3">
          ...
          <div>
            <label className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold mb-1 block">Nominal Diameter</label>
            <div className="relative">
              <input type="text" inputMode="decimal" value={dia} onChange={e => { if (/^[0-9]*\.?[0-9]*$/.test(e.target.value) || e.target.value === "") setDia(e.target.value); }}
              ...
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-gray-600">mm</span>
            </div>
          </div>
          <div>
            <label className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold mb-1.5 block">Standard Fit</label>
            <div className="flex flex-wrap gap-1.5">
              {COMMON_FITS.map(f => (
                <button key={f.label} onClick={() => setSelectedFit(f.label)}
          ...
          {result ? (
            <div className="space-y-0">
              <div className="flex justify-between items-center py-2">
                ...
                <Badge color={fitColor as "green" | "red" | "amber"} className="capitalize">{result.fitType}</Badge>
              </div>
              <div className="py-2 border-t border-dark-700/50">
                <p className="text-[10px] text-gray-600 mb-1">HOLE ({fitDef?.hole}{fitDef?.hg})</p>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Max / Min</span>
                  <span className="font-mono text-white">{fmt(result.holeMax)} / {fmt(result.holeMin)} mm</span>
                </div>
                <div className="flex justify-between text-xs mt-1">
                  <span className="text-gray-600">Deviation</span>
                  <span className="font-mono text-gray-400">+{result.holeUpper} / {result.holeLower} μm</span>
                </div>
              </div>
              <div className="py-2 border-t border-dark-700/50">
                <p className="text-[10px] text-gray-600 mb-1">SHAFT ({fitDef?.shaft}{fitDef?.sg})</p>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Max / Min</span>
                  <span className="font-mono text-white">{fmt(result.shaftMax)} / {fmt(result.shaftMin)} mm</span>
                </div>
                <div className="flex justify-between text-xs mt-1">
                  <span className="text-gray-600">Deviation</span>
                  <span className="font-mono text-gray-400">{result.shaftUpper > 0 ? "+" : ""}{result.shaftUpper} / {result.shaftLower > 0 ? "+" : ""}{result.shaftLower} μm</span>
                </div>
              </div>
              <div className="py-2 border-t border-dark-700/50">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Max Clearance</span>
                  <span className="font-mono text-accent-cyan">{result.maxClearance > 0 ? "+" : ""}{result.maxClearance} μm</span>
                </div>
                <div className="flex justify-between text-sm mt-1">
                  <span className="text-gray-500">{result.minClearance >= 0 ? "Min Clearance" : "Max Interference"}</span>
                  <span className="font-mono text-accent-cyan">{result.minClearance > 0 ? "+" : ""}{result.minClearance} μm</span>
                </div>
              </div>
              <div className="flex justify-between text-xs py-2 border-t border-dark-700/50">
                <span className="text-gray-600">Tolerances</span>
                <span className="font-mono text-gray-400">Hole: {result.holeTolerance}μm · Shaft: {result.shaftTolerance}μm</span>
              </div>
            </div>
...
function GDTTab() {
  ...
  const filtered = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      pool = pool.filter(s => s.name.toLowerCase().includes(q) || s.meaning.toLowerCase().includes(q));
    }
    return pool;
  }, [query, cat]);
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-2">
        <div className="relative flex-1">
          ...
          <input type="text" value={query} onChange={e => setQuery(e.target.value)} placeholder="Search GD&T symbols…"
        ...
        <div className="flex flex-wrap gap-1">
          <button onClick={() => setCat("all")} className={`px-2.5 py-1.5 rounded-lg text-[10px] font-semibold cursor-pointer ${cat === "all" ? "bg-accent-cyan/20 text-accent-cyan" : "text-gray-600 hover:text-white"}`}>All</button>
          {Object.entries(GDT_CATEGORIES).map(([k, label]) => (
            <button key={k} onClick={() => setCat(k)} className={`px-2.5 py-1.5 rounded-lg text-[10px] font-semibold cursor-pointer ${cat === k ? "bg-accent-cyan/20 text-accent-cyan" : "text-gray-600 hover:text-white"}`}>{label}</button>
          ))}
        </div>
      </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <Badge color="gray" className="text-[8px] capitalize">{s.category}</Badge>
                </div>
                <p className="text-xs text-gray-400 mb-2">{s.meaning}</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[10px]">
                  <div><span className="text-gray-600">Application:</span> <span className="text-gray-400">{s.application}</span></div>
                  <div><span className="text-gray-600">Inspection:</span> <span className="text-gray-400">{s.inspection}</span></div>
                </div>
              </div>
            </div>
          </Card>
        ))}
        {filtered.length === 0 && <p className="text-sm text-gray-500 text-center py-8">No symbols found</p>}
      </div>
    </div>
  );
}
      ...
      {PREFERRED_NUMBERS.map(p => (
        <Card key={p.series} variant="solid" padding="md" className="border-dark-600">
          ...
          <div className="flex flex-wrap gap-2">
            {p.values.map(v => (
              <span key={v} className="px-2 py-1 rounded bg-dark-700 font-mono text-sm text-white">{v}</span>
            ))}
          </div>
        </Card>
      ))}
        ...
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`shrink-0 px-4 py-2 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
              tab === t.id ? "bg-accent-blue/20 text-accent-blue border border-accent-blue/30" : "bg-dark-800/60 text-gray-500 border border-dark-700 hover:text-white"}`}>
            {t.label}
          </button>
        ))}
      </div>
