    ...
    <div className="space-y-5 animate-fade-in max-w-4xl mx-auto">
      <PageHeader
        ...
        actions={
          entries.length > 0 ? (
            <button
              onClick={clearAll}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-accent-red bg-accent-red/10 hover:bg-accent-red/20 cursor-pointer transition-all"
            >
      ...
      {entries.length > 0 ? (
        <>
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <div className="flex gap-1 flex-wrap">
              <button
                ...
                className={`px-2.5 py-1.5 rounded-lg text-[10px] font-semibold cursor-pointer ${modFilter === "all" ? "bg-accent-cyan/20 text-accent-cyan" : "text-gray-600 hover:text-white"}`}
              >
                All
              </button>
              {modules.map((m) => (
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-lg text-[10px] font-semibold cursor-pointer capitalize ${modFilter === m ? "bg-accent-cyan/20 text-accent-cyan" : "text-gray-600 hover:text-white"}`}
                >
                  {m}
                </button>
              ))}
              ...
              <div className="space-y-1.5">
                {group.entries.map((entry) => (
                  <Card
                    ...
                    variant="solid"
                    padding="md"
                    className="border-dark-600 group"
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="text-sm text-white truncate">{entry.title}</p>
                          <Badge color="gray" className="text-[8px] shrink-0">
                            {entry.moduleLabel}
                          </Badge>
                        </div>
                        <p className="text-[10px] text-gray-600 mt-0.5">
                          {entry.details || relativeTime(entry.timestamp)}
                        </p>
                      </div>
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() => toggleFavorite(entry.id)}
                          className={`p-1.5 rounded-lg cursor-pointer ${entry.isFavorite ? "text-accent-amber" : "text-gray-600 hover:text-accent-amber"}`}
                        >
          ...
          {filtered.length === 0 && (
            <p className="text-center text-sm text-gray-500 py-8">No matching history</p>
          )}
        </>
      ) : (
