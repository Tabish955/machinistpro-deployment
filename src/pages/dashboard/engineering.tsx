...
import { FunctionSquare, Copy, Check, X, ChevronRight, Info } from "lucide-react";
        ...
        {sigma !== null && p(A) > 0 ? (
          <>
            ...
            <Formula
              ...
              steps={[
                `F = ${F} N`,
                `A = ${A} mm² = ${E.fmt(p(A) * 1e-6)} m²`,
                `σ = ${E.fmt(sigma / 1e6)} MPa`,
              ]}
            />
          </>
        ) : (
...
function BeamCalc() {
  ...
  const moment = valid
    ? isSS
      ? isPoint
        ? E.ssBeamMaxMoment(load, Lm)
        : E.ssUdlMaxMoment(load, Lm)
      : isPoint
        ? E.cantPointMaxMoment(load, Lm)
        : E.cantUdlMaxMoment(load, Lm)
    : 0;
  const defl = valid
    ? isSS
      ? isPoint
        ? E.ssBeamMaxDeflection(load, Lm, Em, Im)
        : E.ssUdlMaxDeflection(load, Lm, Em, Im)
      : isPoint
        ? E.cantPointMaxDeflection(load, Lm, Em, Im)
        : E.cantUdlMaxDeflection(load, Lm, Em, Im)
    : 0;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <Card variant="solid" padding="md" className="border-dark-600 space-y-3">
        ...
        {isPoint ? (
          <Num label="Point Load (P)" value={P} onChange={setP} unit="N" />
        ) : (
          <Num label="Distributed Load (w)" value={w} onChange={setW} unit="kN/m" />
        )}
...
function MoiCalc() {
  ...
  let S = 0;
  let yMax = 0;
  if (shape === "rect" && p(b) > 0 && p(h) > 0) {
    moi = E.moiRectangle(p(b), p(h));
    yMax = p(h) / 2;
    S = E.sectionModulus(moi, yMax);
  }
  if (shape === "circle" && p(D) > 0) {
    moi = E.moiCircle(p(D));
    yMax = p(D) / 2;
    S = E.sectionModulus(moi, yMax);
  }
  if (shape === "hollow" && p(D) > 0 && p(d) > 0) {
    moi = E.moiHollowCircle(p(D), p(d));
    yMax = p(D) / 2;
    S = E.sectionModulus(moi, yMax);
  }
  if (shape === "tri" && p(b) > 0 && p(h) > 0) {
    moi = E.moiTriangle(p(b), p(h));
    yMax = (p(h) * 2) / 3;
    S = E.sectionModulus(moi, yMax);
  }
        ...
        <div className="flex flex-wrap gap-1.5">
          {(
            ...
            <button
              key={k}
              onClick={() => setShape(k)}
              className={`px-3 py-1.5 rounded-lg text-[11px] font-semibold cursor-pointer ${shape === k ? "bg-accent-cyan/20 text-accent-cyan border border-accent-cyan/30" : "bg-dark-700/50 text-gray-500 border border-dark-600 hover:text-white"}`}
            >
              {l}
            </button>
          ))}
        </div>
        {(shape === "rect" || shape === "tri") && (
          <>
            <Num label="Width (b)" value={b} onChange={setB} unit="mm" />
            <Num label="Height (h)" value={h} onChange={setH} unit="mm" />
          </>
        )}
        {shape === "circle" && <Num label="Diameter (D)" value={D} onChange={setD} unit="mm" />}
        {shape === "hollow" && (
          <>
            <Num label="Outer Dia (D)" value={D} onChange={setD} unit="mm" />
            <Num label="Inner Dia (d)" value={d} onChange={setDi} unit="mm" />
          </>
        )}
      </Card>
      <Card variant="solid" padding="md" className="border-dark-600">
      ...
      <Card variant="solid" padding="md" className="border-dark-600">
        ...
        {T !== null ? (
          <>
            ...
            <Formula
              formula="T = (P × 60) / (2π × N)"
              steps={[`P = ${E.fmt(Pw)} W`, `N = ${n} RPM`, `T = ${E.fmt(T)} N·m`]}
            />
          </>
        ) : (
          <p className="text-sm text-gray-500 py-6 text-center">Enter values</p>
      ...
      <Card variant="solid" padding="md" className="border-dark-600">
        ...
        {tau !== null ? (
          <>
            ...
            {phi !== null && (
              <Row label="Angle of Twist" value={E.fmt((phi * 180) / Math.PI, 3)} unit="°" accent />
            )}
      ...
      <Card variant="solid" padding="md" className="border-dark-600">
        ...
        {At !== null ? (
          <>
            ...
            {Tt !== null && (
              <Row label="Tightening Torque" value={E.fmt(Tt / 1000)} unit="N·m" accent />
            )}
...
function FluidCalc() {
  ...
  const Re =
    v !== null && Dm > 0 && p(rho) > 0 && p(mu) > 0 ? E.reynoldsNumber(p(rho), v, Dm, p(mu)) : null;
  const regime =
    Re !== null ? (Re < 2300 ? "Laminar" : Re > 4000 ? "Turbulent" : "Transition") : null;
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <Card variant="solid" padding="md" className="border-dark-600 space-y-3">
      ...
      <Card variant="solid" padding="md" className="border-dark-600">
        ...
        {Qj !== null ? (
          <>
            ...
            <Formula
              formula="Q = m × c × ΔT"
              steps={[`m = ${m} kg`, `c = ${c} J/(kg·K)`, `ΔT = ${dT} K`, `Q = ${E.fmt(Qj)} J`]}
            />
          </>
        ) : (
          <p className="text-sm text-gray-500 py-6 text-center">Enter values</p>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <Card variant="solid" padding="md" className="border-dark-600 space-y-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {(
            ...
            <button
              key={k}
              onClick={() => setMode(k)}
              className={`px-3 py-1.5 rounded-lg text-[11px] font-semibold cursor-pointer ${mode === k ? "bg-accent-cyan/20 text-accent-cyan border border-accent-cyan/30" : "bg-dark-700/50 text-gray-500 border border-dark-600 hover:text-white"}`}
            >
              {l}
            </button>
          ))}
        ...
        {mode === "fos" && (
          <>
            <Num label="Ultimate Strength" value={ult} onChange={setUlt} unit="MPa" />
            <Num label="Actual Stress" value={act} onChange={setAct} unit="MPa" />
          </>
        )}
        {mode === "bearing" && (
          <>
            <Num label="Dynamic Rating (C)" value={Cb} onChange={setCb} unit="kN" />
            <Num label="Applied Load (P)" value={Pb} onChange={setPb} unit="kN" />
            <Num label="Speed" value={nb} onChange={setNb} unit="RPM" />
          </>
        )}
        {mode === "gear" && (
          <>
            <Num label="Driver Teeth (N₁)" value={N1} onChange={setN1} />
            <Num label="Driven Teeth (N₂)" value={N2} onChange={setN2} />
          </>
        )}
        {mode === "belt" && (
          <>
            <Num label="Pulley Diameter" value={Db} onChange={setDb} unit="mm" />
            <Num label="Speed" value={nb2} onChange={setNb2} unit="RPM" />
          </>
        )}
      </Card>
      <Card variant="solid" padding="md" className="border-dark-600">
        ...
        {mode === "fos" && p(act) > 0 && p(ult) > 0 ? (
          ...
          <>
            ...
            {p(nb) > 0 && (
              <Row
                label="L₁₀ Hours"
                value={E.fmt(E.bearingLife(p(Cb), p(Pb)) / (p(nb) * 60))}
                unit="hrs"
              />
            )}
