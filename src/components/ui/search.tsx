  ...
  for (const f of FORMULAS) {
    ...
    if (s > 0)
      results.push({
        id: `form-${f.id}`,
        label: f.name,
        description: formatMath(f.expression),
  ...
  for (const t of THREAD_DB) {
    if (t.size.toLowerCase().includes(q) || t.standard.toLowerCase().includes(q)) {
      results.push({
        id: `thr-${t.size}`,
        label: t.size,
        description: `${t.standard} · Tap: ${t.tapDrill}mm`,
        href: "/dashboard/materials",
        category: "Threads",
        icon: "thread",
        score: 50,
  ...
  for (const c of ENGINEERING_CONSTANTS) {
    if (c.name.toLowerCase().includes(q) || c.symbol.toLowerCase().includes(q)) {
      results.push({
        id: `const-${c.id}`,
        label: c.name,
        description: `${c.symbol} = ${c.value} ${c.unit}`,
        href: "/dashboard/formulas",
        category: "Constants",
        icon: "constant",
        score: 40,
...
export function GlobalSearch({
  ...
  const quickActions = useMemo(
    () => allModules.filter((m) => m.status === "available").slice(0, 6),
    [],
  );
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[12vh] sm:pt-[15vh] px-4">
          ...
          <div
            ref={containerRef}
            className="relative w-full max-w-lg rounded-2xl border border-dark-600 bg-dark-800 shadow-2xl shadow-black/60 overflow-hidden animate-scale-in"
          >
            {/* Input */}
            <div className="flex items-center gap-3 px-4 py-3.5 border-b border-dark-700">
              ...
              <kbd className="px-1.5 py-0.5 rounded bg-dark-700 text-[10px] font-mono text-gray-600">
                ESC
              </kbd>
            </div>
            ...
            <div className="max-h-[50vh] overflow-y-auto p-2">
              {query.trim() ? (
                  ...
                  <>
                    ...
                    {(() => {
                      ...
                      for (const r of results) {
                        const a = groups.get(r.category) ?? [];
                        a.push(r);
                        groups.set(r.category, a);
                      }
                      let idx = 0;
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  <p
                                    className={`text-sm font-medium truncate ${thisIdx === selectedIndex ? "text-white" : "text-gray-300"}`}
                                  >
                                    {item.label}
                                  </p>
                                ...
                                <ArrowRight
                                  size={12}
                                  className={
                                    thisIdx === selectedIndex ? "text-accent-cyan" : "text-dark-500"
                                  }
                                />
                              </button>
                            );
                ...
                /* Quick actions */
                <div>
                  <p className="text-[10px] text-gray-600 uppercase tracking-wider px-3 pt-1 pb-1.5">
                    Quick Actions
                  </p>
            ...
            <div className="flex items-center justify-between px-4 py-2.5 border-t border-dark-700 bg-dark-900/50 text-[10px] text-gray-600">
              <div className="flex items-center gap-3">
                <span className="flex items-center gap-1">
                  <kbd className="px-1 py-0.5 rounded bg-dark-700">↑↓</kbd> Navigate
                </span>
                <span className="flex items-center gap-1">
                  <kbd className="px-1 py-0.5 rounded bg-dark-700">↵</kbd> Open
                </span>
              </div>
              <span>
                {query.trim()
