...
const length: CategoryDef = {
  id: "length", name: "Length", icon: "Ruler", baseUnit: "m", group: "basic",
  units: [
...
const area: CategoryDef = {
  id: "area", name: "Area", icon: "Square", baseUnit: "m2", group: "basic",
  units: [
...
const volume: CategoryDef = {
  id: "volume", name: "Volume", icon: "Box", baseUnit: "m3", group: "basic",
  units: [
...
const mass: CategoryDef = {
  id: "mass", name: "Mass", icon: "Weight", baseUnit: "kg", group: "basic",
  units: [
...
const time: CategoryDef = {
  id: "time", name: "Time", icon: "Clock", baseUnit: "s", group: "basic",
  units: [
...
const temperature: CategoryDef = {
  id: "temperature", name: "Temperature", icon: "Thermometer", baseUnit: "K",
  group: "basic",
  units: [
    { id: "K",  name: "Kelvin",     symbol: "K",  aliases: ["kelvin"],
      toBase: 1, fromBase: 1 },
    { id: "C",  name: "Celsius",    symbol: "°C", aliases: ["celsius", "centigrade"],
      toBase: (v: number) => v + 273.15, fromBase: (v: number) => v - 273.15 },
    { id: "F",  name: "Fahrenheit", symbol: "°F", aliases: ["fahrenheit"],
      toBase: (v: number) => (v - 32) * 5 / 9 + 273.15,
      fromBase: (v: number) => (v - 273.15) * 9 / 5 + 32 },
    { id: "Ra", name: "Rankine",    symbol: "°Ra", aliases: ["rankine"],
      toBase: (v: number) => v * 5 / 9, fromBase: (v: number) => v * 9 / 5 },
  ],
};

const speed: CategoryDef = {
  id: "speed", name: "Speed", icon: "Gauge", baseUnit: "mps", group: "basic",
  units: [
...
const pressure: CategoryDef = {
  id: "pressure", name: "Pressure", icon: "ArrowDownUp", baseUnit: "Pa",
  group: "basic",
  units: [
...
const force: CategoryDef = {
  id: "force", name: "Force", icon: "ArrowRight", baseUnit: "N", group: "basic",
  units: [
...
const torque: CategoryDef = {
  id: "torque", name: "Torque", icon: "RotateCcw", baseUnit: "Nm", group: "basic",
  units: [
...
const energy: CategoryDef = {
  id: "energy", name: "Energy", icon: "Zap", baseUnit: "J", group: "basic",
  units: [
...
const power: CategoryDef = {
  id: "power", name: "Power", icon: "Activity", baseUnit: "W", group: "basic",
  units: [
    u("W",     "Watt",             "W",      1,           ["watts"]),
...
const density: CategoryDef = {
  id: "density", name: "Density", icon: "Layers", baseUnit: "kgm3", group: "basic",
  units: [
    u("kgm3",  "kg/m³",           "kg/m³",  1,           []),
...
const frequency: CategoryDef = {
  id: "frequency", name: "Frequency", icon: "Radio", baseUnit: "Hz", group: "basic",
  units: [
...
const angle: CategoryDef = {
  id: "angle", name: "Angle", icon: "Triangle", baseUnit: "deg", group: "basic",
  units: [
...
const acceleration: CategoryDef = {
  id: "acceleration", name: "Acceleration", icon: "TrendingUp", baseUnit: "mps2",
  group: "basic",
  units: [
    u("mps2",  "m/s²",            "m/s²",   1,           []),
...
const flowRate: CategoryDef = {
  id: "flowrate", name: "Flow Rate", icon: "Droplets", baseUnit: "m3s",
  group: "engineering",
  units: [
    u("m3s",   "m³/s",            "m³/s",   1,           []),
...
const dynVisc: CategoryDef = {
  id: "dynviscosity", name: "Dynamic Viscosity", icon: "Droplet",
  baseUnit: "Pas", group: "engineering",
  units: [
...
const kinVisc: CategoryDef = {
  id: "kinviscosity", name: "Kinematic Viscosity", icon: "Waves",
  baseUnit: "m2s", group: "engineering",
  units: [
    u("m2s",   "m²/s",            "m²/s",  1,       []),
    u("St",    "Stokes",          "St",    1e-4,    []),
...
const thermCond: CategoryDef = {
  id: "thermalcond", name: "Thermal Conductivity", icon: "Flame",
  baseUnit: "WmK", group: "engineering",
  units: [
    u("WmK",   "W/(m·K)",         "W/(m·K)",  1,         []),
...
const stress: CategoryDef = {
  id: "stress", name: "Stress / Pressure", icon: "Maximize2",
  baseUnit: "Pa_s", group: "engineering",
  units: [
    u("Pa_s",  "Pascal",          "Pa",    1,           []),
...
const momentInertia: CategoryDef = {
  id: "momentinertia", name: "Moment of Inertia", icon: "Circle",
  baseUnit: "m4", group: "engineering",
  units: [
    u("m4",    "m⁴",              "m⁴",    1,         []),
    u("cm4",   "cm⁴",             "cm⁴",   1e-8,      []),
    u("mm4",   "mm⁴",             "mm⁴",   1e-12,     []),
...
const fuelConsumption: CategoryDef = {
  id: "fuelconsumption", name: "Fuel Consumption", icon: "Fuel",
  baseUnit: "lp100km", group: "engineering",
  units: [
    u("lp100km","L/100km",        "L/100km",1,          []),
    { id: "mpgUS", name: "MPG (US)", symbol: "mpg", aliases: ["miles per gallon"],
      toBase: (v: number) => v === 0 ? Infinity : 235.2145833 / v,
      fromBase: (v: number) => v === 0 ? Infinity : 235.2145833 / v },
    { id: "mpgImp", name: "MPG (Imperial)", symbol: "mpg (imp)", aliases: [],
...
const voltage: CategoryDef = {
  id: "voltage", name: "Voltage", icon: "Zap", baseUnit: "V", group: "electrical",
  units: [
    u("V",     "Volt",            "V",     1,        ["volts"]),
...
const current: CategoryDef = {
  id: "current", name: "Current", icon: "Activity", baseUnit: "A",
  group: "electrical",
  units: [
...
const resistance: CategoryDef = {
  id: "resistance", name: "Resistance", icon: "Omega", baseUnit: "ohm",
  group: "electrical",
  units: [
    u("ohm",   "Ohm",             "Ω",     1,        ["ohms"]),
...
const capacitance: CategoryDef = {
  id: "capacitance", name: "Capacitance", icon: "Battery", baseUnit: "F",
  group: "electrical",
  units: [
...
const inductance: CategoryDef = {
  id: "inductance", name: "Inductance", icon: "Magnet", baseUnit: "H",
  group: "electrical",
  units: [
...
const charge: CategoryDef = {
  id: "charge", name: "Electric Charge", icon: "CircleDot", baseUnit: "C",
  group: "electrical",
  units: [
...
const dataSize: CategoryDef = {
  id: "datasize", name: "Data Size", icon: "HardDrive", baseUnit: "B",
  group: "computing",
  units: [
...
const dataRate: CategoryDef = {
  id: "datarate", name: "Data Rate", icon: "Wifi", baseUnit: "bps",
  group: "computing",
  units: [
...
export const ALL_CATEGORIES: CategoryDef[] = [
  // Basic
  length, area, volume, mass, time, temperature, speed, pressure,
  force, torque, energy, power, density, frequency, angle, acceleration,
  // Engineering
  flowRate, dynVisc, kinVisc, thermCond, stress, momentInertia, fuelConsumption,
  // Electrical
